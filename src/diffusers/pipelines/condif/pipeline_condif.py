...
from ...models import AutoencoderKL, CondifModel, ImageProjection, UNet2DConditionModel
...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        from diffusers import StableDiffusionCondifPipeline, CondifModel, UniPCMultistepScheduler
        ...
        condif_path = "ckpt_path"

        condif = CondifModel.from_pretrained(condif_path, torch_dtype=torch.float16)
        pipe = StableDiffusionCondifPipeline.from_pretrained(
            base_model_path, condif=condif, torch_dtype=torch.float16, low_cpu_mem_usage=False
        )
        ...
        image_path="examples/condif/test_image.jpg"
        mask_path="examples/condif/test_mask.jpg"
...
def load_skeleton_npz(npz_path):
    ...
    Ty /= mag
    return S, Tx, Ty
class StableDiffusionCondifPipeline(
    DiffusionPipeline,
    ...
    r"""
    Pipeline for structure-aware inpainting using Stable Diffusion with ConDiF guidance.
    ...
    Args:
        ...
        condif ([`CondifModel`]):
            Structure guidance branch injected into the frozen UNet during denoising.
        scheduler ([`SchedulerMixin`]):
    ...
    def __init__(
        ...
        if condif is None:
            raise ValueError("`condif` must be provided.")

        guidance_model = condif
    ...
    def check_inputs(
        ...
        is_compiled = hasattr(F, "scaled_dot_product_attention") and isinstance(
            self.condif, torch._dynamo.eval_frame.OptimizedModule
        )
        if (
            isinstance(self.condif, CondifModel)
            or is_compiled
            and isinstance(self.condif._orig_mod, CondifModel)
        ):
            self.check_image(image, mask, prompt, prompt_embeds)
        else:
            assert False

        # Check `condif_conditioning_scale`
        if (
            isinstance(self.condif, CondifModel)
            or is_compiled
            and isinstance(self.condif._orig_mod, CondifModel)
        ):
            if not isinstance(condif_conditioning_scale, float):
                raise TypeError("For ConDiF: `condif_conditioning_scale` must be type `float`.")
        else:
            assert False
    ...
    def __call__(
        ...
        condif_conditioning_scale: Union[float, List[float]] = 1.0,
        ...
        skeleton_npz_paths: Optional[List[str]] = None,
        **kwargs,
    ):
        r"""
        ...
        Args:
            ...
            image (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, `List[np.ndarray]`,:
                ...
                The corrupted input image for inpainting. White regions in `mask` indicate areas to fill.
            mask (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, `List[np.ndarray]`,:
                    `List[List[torch.FloatTensor]]`, `List[List[np.ndarray]]` or `List[List[PIL.Image.Image]]`):
                Binary mask for the inpainting region (white = inpaint).
            ...
            condif_conditioning_scale (`float` or `List[float]`, *optional*, defaults to 1.0):
                The outputs of ConDiF are multiplied by `condif_conditioning_scale` before they are added
                to the residual in the original `unet`. If multiple guidance branches are specified in `init`, you can set
                the corresponding scale as a list.
            guess_mode (`bool`, *optional*, defaults to `False`):
                In guess mode, ConDiF runs only on the conditional batch and scales guidance adaptively.
                A `guidance_scale` value between 3.0 and 5.0 is recommended when using guess mode.
            control_guidance_start (`float` or `List[float]`, *optional*, defaults to 0.0):
                The percentage of total steps at which ConDiF guidance starts applying.
            control_guidance_end (`float` or `List[float]`, *optional*, defaults to 1.0):
                The percentage of total steps at which ConDiF guidance stops applying.
            clip_skip (`int`, *optional*):
                Number of layers to be skipped from CLIP while computing the prompt embeddings. A value of 1 means that
                the output of the pre-final layer will be used for computing the prompt embeddings.
        ...
        condif_model = self.condif._orig_mod if is_compiled_module(self.condif) else self.condif
        ...
        global_pool_conditions = (
            condif_model.config.global_pool_conditions
            if isinstance(condif_model, CondifModel)
            else condif_model.nets[0].config.global_pool_conditions
        )
        ...
        if isinstance(condif_model, CondifModel):
            ...
            original_mask = (original_mask.mean(dim=1, keepdim=True) > 0).to(image.dtype)
        ...
        if skeleton_npz_paths is not None:
            ...
            Tx_list = []
            Ty_list = []
            for path in skeleton_npz_paths:
                S, Tx, Ty = load_skeleton_npz(path)
                S_list.append(S)
                Tx_list.append(Tx)
                Ty_list.append(Ty)
        ...
        # 7.2 Create tensor stating which ConDiF steps to keep
        condif_keep = []
        for i in range(len(timesteps)):
            ...
            condif_keep.append(keeps[0] if isinstance(condif_model, CondifModel) else keeps)

        # 8. Denoising loop
        num_warmup_steps = len(timesteps) - num_inference_steps * self.scheduler.order
        is_unet_compiled = is_compiled_module(self.unet)
        is_condif_compiled = is_compiled_module(self.condif)
        is_torch_higher_equal_2_1 = is_torch_version(">=", "2.1")
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                # Relevant thread:
                # https://dev-discuss.pytorch.org/t/cudagraphs-in-pytorch-2-0/1428
                if (is_unet_compiled and is_condif_compiled) and is_torch_higher_equal_2_1:
                ...
                if guess_mode and self.do_classifier_free_guidance:
                    # Run ConDiF only for the conditional batch.
                    control_model_input = latents
                    control_model_input = self.scheduler.scale_model_input(control_model_input, t)
                    condif_prompt_embeds = prompt_embeds.chunk(2)[1]
                else:
                    control_model_input = latent_model_input
                    condif_prompt_embeds = prompt_embeds

                if isinstance(condif_keep[i], list):
                    cond_scale = [c * s for c, s in zip(condif_conditioning_scale, condif_keep[i])]
                else:
                    condif_scale = condif_conditioning_scale
                    if isinstance(condif_scale, list):
                        condif_scale = condif_scale[0]
                    cond_scale = condif_scale * condif_keep[i]

                down_block_res_samples, mid_block_res_sample, up_block_res_samples = self.condif(
                    control_model_input,
                    t,
                    encoder_hidden_states=condif_prompt_embeds,
                ...
                if guess_mode and self.do_classifier_free_guidance:
                    # ConDiF outputs apply to the conditional batch; pad unconditional with zeros.
        ...
        # If we do sequential model offloading, let's offload unet and condif
