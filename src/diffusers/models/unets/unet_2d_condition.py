...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin, PeftAdapterMixin):
    ...
    def _register_debug_hooks(self):
        ...
        def hook_fn(module, input, output):
            ...
            # 安全地处理输入输出
            input_shapes = tuple(
                x.shape if isinstance(x, torch.Tensor) else type(x).__name__ for x in input
            )
            if isinstance(output, torch.Tensor):
                print(f"[HOOK] {cls_name} | Input: {input_shapes} -> Output: {output.shape}")
            elif isinstance(output, (tuple, list)) and isinstance(output[0], torch.Tensor):
                print(f"[HOOK] {cls_name} | Output[0]: {output[0].shape}")
            else:
                print(f"[HOOK] {cls_name} | Output type: {type(output)}")

        # ✅ 在 UNet 中注册常用模块
        for name, module in self.named_modules():
            if any(x in name.lower() for x in ["resnet", "attn", "down", "up", "conv", "norm"]):
    ...
    def forward(
       ...
       #self._register_debug_hooks()
        ...
        is_condif_guidance = down_block_add_samples is not None and mid_block_add_sample is not None and up_block_add_samples is not None
        ...
        for downsample_block in self.down_blocks:
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                ...
                if is_condif_guidance and len(down_block_add_samples)>0:
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                additional_residuals = {}
                if is_condif_guidance and len(up_block_add_samples)>0:
